...
function toRelativePath(filePath: string): string {
  ...
  // Common prefixes to strip for sandbox paths
...
export interface ToolResultItemProps {
  ...
  /** Tool execution state - shows loading indicator when not output-available */
  state?: ToolState
}
      ...
      case "Created":
        return (
          <svg
            ...
            className="shrink-0 h-4 w-4 text-gray-500"
            fill="currentColor"
          >
  ...
  return (
    <div className="mb-2">
      ...
      >
        ...
        <span className="flex-shrink-0 font-normal text-gray-400">
          {action}
        </span>
        <span
          className="relative w-fit max-w-xs truncate rounded-md bg-white/10 px-2 py-0 text-start text-xs font-normal text-gray-400 transition-colors hover:bg-white/20"
          title={displayPath}
        >
        ...
        {hasContent && (
          <svg
            ...
            className={`ml-1 text-gray-500 transition-transform ${isExpanded ? "rotate-90" : ""}`}
          >
