...
const DEFAULT_ACTIVE_FILE_PATHS = [
  "app/page.tsx",
  "src/App.tsx",
  "index.js",
  "package.json",
...
const getFileIcon = (name: string) => {
  ...
  if (name.endsWith(".css"))
    return <FileType className="h-4 w-4 text-blue-300" />;
  if (name.endsWith(".html"))
    return <FileCode className="h-4 w-4 text-orange-400" />;
  if (name.endsWith(".json"))
    return <FileJson className="h-4 w-4 text-yellow-200" />;
  if (name.endsWith(".md"))
    return <FileText className="h-4 w-4 text-gray-400" />;
  if (name.match(/\.(png|jpg|jpeg|svg|gif)$/))
...
const FileTreeNode = ({
  ...
  const [isOpen, setIsOpen] = React.useState(
    defaultOpen || OPEN_BY_DEFAULT_FOLDERS.has(node.name),
  );
    ...
    <div>
      <div
        className={cn(
          ...
          activePath === node.path && node.type === "file"
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <span className="text-xs text-zinc-600">
                      Files will appear here after the project is created
                    </span>
                  </div>
                )}
              </div>
