...
async function attemptScreenshotCapture(
  name: string,
  url: string,
  projectId: string | undefined,
  saveScreenshot: (base64: string, url?: string) => Promise<boolean>,
  ...
  try {
    ...
    if (retryCount < maxRetries) {
      ...
      return attemptScreenshotCapture(name, url, projectId, saveScreenshot, retryCount + 1, maxRetries);
    }
...
export function useScreenshotCapture({
  ...
  const captureAndSaveScreenshot = useCallback(
    async (name: string, url: string, retryCount = 0) => {
      return attemptScreenshotCapture(name, url, projectIdRef.current, saveScreenshotRef.current, retryCount, 2);
    },
    [],
  );
