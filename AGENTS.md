...
| Sandbox Lifecycle | `lib/e2b/sandbox.ts`          | Sandbox state managed via in-memory Maps (`activeSandboxes`, `pausedSandboxes`, `connectionAttempts`). Database-backed persistence for sandbox IDs via `ProjectRepository`. Supports create, pause, resume, close, and auto-cleanup with connection retry logic.      |
...
- Lifecycle managed via in-memory Maps (`activeSandboxes`, `pausedSandboxes`, `connectionAttempts`) in `lib/e2b/sandbox.ts`, with database-backed persistence for sandbox IDs.
