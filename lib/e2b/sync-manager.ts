...
const DEFAULT_CONFIG: Required<SyncManagerConfig> = {
  ...
  ignorePaths: [
    ...
    "bun.lock",
    "yarn.lock",
    // Sandbox home/system artifacts (should never be part of a project snapshot)
    ".bashrc",
    ".profile",
    ".bash_logout",
...
export class SyncManager {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      `[SyncManager] Initialized for project ${this.config.projectId}`,
    );
  }
  ...
  private async syncDbToSandbox(): Promise<SyncResult> {
    ...
    const dirs = getRequiredDirectories(
      Object.keys(snapshot).map((p) => `${this.config.projectDir}/${p}`),
    );
   ...
   */
  private async saveSnapshotToDb(
    snapshot: Record<string, string>,
  ): Promise<boolean> {
    try {
      ...
      await projectCache.invalidate(this.config.projectId);
      console.log(
        `[SyncManager] Saved ${Object.keys(snapshot).length} files to database for project ${this.config.projectId}`,
      );
      return true;
    } catch (error) {
  ...
  private handleFileChanges(changes: FileChange[]): void {
    ...
    const relevantChanges = changes.filter(
      (c) =>
        !this.shouldIgnore(getRelativePath(c.path, this.config.projectDir)),
    );
   ...
   */
  private static patternRegexCache = new Map<string, RegExp>();
  private matchPattern(path: string, pattern: string): boolean {
    let regex = SyncManager.patternRegexCache.get(pattern);
    if (!regex) {
      ...
      SyncManager.patternRegexCache.set(pattern, regex);
    }
...
export async function quickSyncToDatabaseWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      retryCount = attempt + 1;
      console.warn(
        `[SyncManager] Sync attempt ${attempt + 1} failed:`,
        lastError.message,
      );
  ...
  return {
    ...
    duration: 0,
    errors: [
      { path: "", error: lastError?.message || "Sync failed after retries" },
    ],
    retryCount,
  };
}
