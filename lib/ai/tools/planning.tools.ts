  ...
  return {
    ...
    planChanges: tool({
      description:
        ...
        "Include accessibility (semantic HTML, ARIA), performance (code-splitting, " +
        "image optimization), and responsive design in your plan.",
      inputSchema: z.object({
        goal: z
          ...
          .max(500)
          .describe(
            "The overall goal or feature to implement - be specific and measurable",
          ),
    ...
    markStepComplete: tool({
      ...
      inputSchema: z.object({
        step: z
          .string()
          .min(1)
          .describe("Description of the completed step (should match plan)"),
        notes: z
          .string()
          .max(500)
          .optional()
          .describe(
            "Optional notes about implementation details or issues encountered",
          ),
      }),
      execute: async ({ step, notes }) => {
        ...
        try {
          ...
          const remainingSteps =
            context.currentPlan?.slice(completedCount) ?? [];
